...
const femTbody = document.querySelector("#fem-tbl > table > tbody");
const element = document.getElementById("fem-tbl");
...
function renderStudents(students){
    ...
    students.forEach((student)=>{
        ...
        tr.innerHTML=`<th scope="row">${student.id}</th>
        <td class="img-cls"><img src=${student.img_src} alt="profile_img"> ${student.first_name} ${student.last_name}</td>
...
function renderStudents2(students){
    femTbody.innerHTML='';
    students.forEach((student)=>{
        const tr = document.createElement("tr");
        tr.innerHTML=`<th scope="row">${student.id}</th>
...
btn.addEventListener("click",()=>{
    ...
    element.classList.remove("d-block");
     ...
     const arr1 = students.filter((student)=>{
        return student.first_name.toLowerCase().includes(val) ;

     });
     const arr2 = students.filter((student)=>{
        return student.last_name.toLowerCase().includes(val) ;

     });
     const arr3 = students.filter((student)=>{
        return student.email.toLowerCase().includes(val) ;
...
items[0].addEventListener("click",()=>{
    ...
    students.sort((a, b) => {
        let fa = `${a.first_name+a.last_name}`.toLowerCase(),
            fb = `${b.first_name+b.last_name}`.toLowerCase();
